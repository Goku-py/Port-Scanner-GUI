...
class ScannerGUI(tk.Tk):
    ...
    def poll_results(self):
        ...
        # Direct UI updates by reading variables (avoids queue bloat)
        scanned = self.scanner.scanned_count
        total = self.scanner.total_ports
        self.progress.configure(maximum=max(total, 1), value=scanned)
        self.var_status.set(f"Scanning... {scanned}/{total}")

        is_alive = self.scanner_thread and self.scanner_thread.is_alive()
        # Throttled queue reading to prevent GUI locking, unless thread is dead
        max_msgs = 100 if is_alive else 100000 
        msg_count = 0
        ...
        # Continue polling if thread is alive OR queue still has items
        if (self.scanner_thread and self.scanner_thread.is_alive()) or not self.scanner.result_queue.empty():
            self.after(self.poll_after_ms, self.poll_results)
        else:
